...
public class TeleportCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length < 1) {
			player.sendMessage(ColorText.translate("&cUsage: /tp <playerName>"));
		} else {
			...
			if (target.equals(player)) {
				player.sendMessage(ColorText.translate("&cTeleport> &6You cannot teleport to yourself."));
				return true;
			}
      
			TeleportManager.teleportToPlayer(player, target);	   
		}
		return true;
	}
}
